...
void writeLogHeader() {
  if (SD_b) {
    if (SD.begin(2)) {
      ...
      if (SD.exists(def_filename)) {
        SD.remove(def_filename); // Delete the existing file
      }

      File dataFile = SD.open(def_filename, FILE_WRITE);
      if (dataFile && dataFile.size() == 0) {  // Only write if file is empty
        ...
        dataFile.close();
        Serial.print("File initialization successful");
      }
...
void setup() {
  ...
  if(serial){
    ...
    if(response.length() > 0){
      def_filename=response+".csv";
      Serial.println("ack");
      Serial.println(String("Writing log to:")+def_filename);
    }
  }
...
void handleCommand(String cmd) {
  ...
  else {Serial.print("Unrecognized variable: ");Serial.println(var);return;}
  // confirm output
  Serial.print("Set ");
  Serial.print(var);
  Serial.print(" to ");
